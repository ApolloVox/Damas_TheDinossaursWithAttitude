        ...
        public void Update(GameTime gametime)
        {
            ...
            if (keys.IsKeyDown(Keys.NumPad5))
            {
                if (position.X - (float)gametime.ElapsedGameTime.TotalSeconds * 2 > 0)
                    //position.X = MathHelper.Clamp(position.X - (float)gametime.ElapsedGameTime.TotalSeconds * 20f, 0, map.MapBoundariesWidth-1);
            ...
            if (keys.IsKeyDown(Keys.NumPad4))
            {
                if (position.Z - (float)gametime.ElapsedGameTime.TotalSeconds * 2 > 0)
                    position.Z = MathHelper.Clamp(position.Z - (float)gametime.ElapsedGameTime.TotalSeconds * 20f, 0, map.MapBoundariesHeight-1);
                else
                    position.Z = 0;
            }
